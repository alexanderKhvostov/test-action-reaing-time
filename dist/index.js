...
;(() => {
  ...
  ;(() => {
    ...
    } // CONCATENATED MODULE: ./src/manifestHelpers.mjs

    async function fetchDocsManifest(path) {
      const res = await getFile(path)
      return JSON.parse(res)
    }

    async function updateManifestRoutes(manifest, routes) {
    ...
    var reading_time = __nccwpck_require__(820) // CONCATENATED MODULE: ./src/calculateRoutesReadingTime.mjs
    /* harmony default export */ function calculateRoutesReadingTime(routes) {
      return routes.map((mainRoute) => {
        return {
          ...mainRoute,
          routes: mainRoute.routes.map(async (subRoute) => {
            const { content } = await getFile(subRoute.path)
            return { ...subRoute, readingTime: reading_time(content) }
          })
        }
      })
